...
const handleHistoryPerUid = distinctUntilChanged(
  handleHistory,
  (prev, curr) => prev?.attributes?.uid === curr.attributes.uid,
)
...
export function distinctUntilChanged<TData>(
  fn: (val: TData) => void,
  compareFn?: (prev: TData | undefined, curr: TData) => boolean,
) {
  let prev: TValue<TData> | undefined
  return (value: TData): void => {
    if (prev === undefined || (!Object.is(value, prev) && !compareFn)) {
      fn(value)
      prev = value
    }
    if (compareFn && typeof compareFn === 'function' && !compareFn(prev, value)) {
      fn(value)
      prev = value
    }
  }
...
More about sendEncodedMessage example